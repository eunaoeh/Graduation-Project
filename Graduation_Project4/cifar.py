...
def main():
    ...
    if opt.teacher_id:
        ...
        def f(inputs, params, mode):
            ...
            # 처음 epoch에서 학생의 3번째 attention map을 normalize한 후 파일에 저장
            if c_epoch == 1:
                def memo(filename, g_s):
                    if len(g_s) == opt.batch_size:
                        f = open(filename, 'a')
                        for i in range(g_s.size(0)):
                            f.write(str(g_s[i].mean(0).cpu().detach().numpy()))
                            f.write(', ')
...
#                for i in range(g_s[2].size(0)):
#                    print(g_s[2][i].mean(0).cpu().detach().numpy())

#                print(replace(g_s[2], 2, at_idx))
#                print("------------------------------------")
                memo("g_s[1].txt", g_s[1])
                r = replace(g_s[1], 1, at_idx)
                f = open("g_s_1.txt", 'a')
                f.write(str(r))
    ...
    def on_start_epoch(state):
        ...
        if epoch == 2:
            ...
            cluster_idx_1, cluster_centers_1 = utils.clustering("resnet_16_2_1.txt", n, 16, opt.batch_size)
            cluster_idx_2, cluster_centers_2 = utils.clustering("resnet_16_2_2.txt", n, 8, opt.batch_size)
            #cluster_centers_0 = cluster_centers_0.reshape(n, 128, 1024)
            print(cluster_centers_1.shape, cluster_centers_2.shape)
            print(cluster_idx_1.shape, cluster_idx_2.shape)
            cluster_centers_1 = cluster_centers_1.reshape(n, 16, 16)
            cluster_centers_2 = cluster_centers_2.reshape(n, 8, 8)
            ...
            def masking(cluster_centers):
                ...
                for center in cluster_centers:
                    ...
                    for i in range(size[1] - (size[1]//4), size[1]):
                        for j in range(size[2]):
                            center[i][j] = 0

                    for j in range(size[2]//4):
                        for i in range(size[1]):
                            center[i][j] = 0

                    for j in range(size[2] - (size[2]//4), size[2]):
                        for i in range(size[1]):
                            center[i][j] = 0
            ...
            #print(cluster_centers_1, cluster_centers_2)
